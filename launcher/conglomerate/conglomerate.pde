...
void setup() {
  ...
  // Initialize menu
  menuFont = createFont("Arial Bold", 48);
...
void drawMenu() {
  ...
  drawMenuCard(leftCardX, cardY, menuCardWidth, menuCardHeight, menuFlowersImg, "Water the Plants", 0);
  
  // Right card (Cards)
  float rightCardX = startX + menuCardWidth + menuCardSpacing;
  drawMenuCard(rightCardX, cardY, menuCardWidth, menuCardHeight, menuCardsImg, "Memory Match", 1);
}

void drawMenuCard(float x, float y, float w, float h, PImage img, String label, int cardIndex) {
...
void handleMenuClick() {
  ...
  // Check left card (Garden)
  float leftCardX = startX;
  if (mouseX >= leftCardX && mouseX <= leftCardX + menuCardWidth && 
      mouseY >= cardY && mouseY <= cardY + menuCardHeight) {
    currentState = GameState.GARDEN;
    return;
  }
  
  // Check right card (Cards)
  float rightCardX = startX + menuCardWidth + menuCardSpacing;
  if (mouseX >= rightCardX && mouseX <= rightCardX + menuCardWidth && 
      mouseY >= cardY && mouseY <= cardY + menuCardHeight) {
...
void cards_updateLayoutMetrics() {
  float scaleX = cards_viewWidth / cards_DESIGN_WIDTH;
  float scaleY = cards_viewHeight / cards_DESIGN_HEIGHT;
  cards_layoutScale = min(scaleX, scaleY);
  cards_layoutScale = min(cards_layoutScale, 1.0f);
  ...
  float availableWidth = max(cards_viewWidth - cards_paddingX * 2f, 0);
  float availableHeight = max(cards_viewHeight - cards_paddingY * 2f, 0);
  float offsetX = max(availableWidth - cards_totalGridWidth, 0) / 2f;
  float offsetY = max(availableHeight - cards_totalGridHeight, 0) / 2f;
